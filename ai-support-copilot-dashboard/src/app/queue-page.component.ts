    ...
    <section class="split" style="margin-top: 0.9rem">
      <eds-card class="card-pad" [elevated]="false">
        <div class="section-head">
          ...
          <eds-tag label="8 open" variant="brand"></eds-tag>
        </div>
      ...
      <eds-card class="card-pad" [elevated]="false">
        ...
        <p class="meta">Scored on Ananya Poluru’s live support set.</p>
...
export class QueuePageComponent {
  ...
  protected readonly ticketRows = this.config.conversations.slice(0, 5).map((entry) => ({
    id: entry.id,
    topic: entry.topic,
    owner: entry.owner,
    status: entry.status,
  ...
  protected statusVariant(status: string): 'success' | 'warning' | 'danger' | 'info' | 'neutral' {
    if (status === 'Resolved' || status === 'Ready' || status === 'Active') {
      return 'success';
    }
