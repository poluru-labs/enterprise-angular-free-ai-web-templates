...
type AlertRow = (typeof templateConfig.alerts)[number] & { done?: boolean };
  ...
  template: `
    <section class="page-head">
      <div>
        ...
        <h1>Alerts</h1>
        <p class="summary">Triage high-risk signals. Assign, snooze, or open the linked case.</p>
      </div>
    ...
    <section class="metrics mini">
      ...
      <article class="metric"><p>Median age</p><div class="value">12m</div></article>
      <article class="metric"><p>Assigned to you</p><div class="value">1</div></article>
    </section>

    <div class="toolbar">
      <label class="search">
        <span class="material-symbols-outlined">search</span>
        <input type="search" placeholder="Search person, BIN, or rule" (input)="query.set($any($event.target).value)" />
      </label>
      <div class="chips">
...
export class AlertsPageComponent {
  ...
  protected readonly filtered = computed(() => {
    ...
    return this.rows().filter((item) => {
      ...
      const qOk = !q || `${item.title} ${item.detail} ${item.assignee}`.toLowerCase().includes(q);
      return sevOk && qOk && !item.done;
    });
  });

  protected readonly selected = computed(() => this.filtered().find((item) => item.id === this.selectedId()) ?? this.filtered()[0]);
  protected readonly openCount = computed(() => this.rows().filter((item) => !item.done).length);
  protected readonly highCount = computed(() => this.rows().filter((item) => !item.done && item.severity === 'High').length);
